...
def predict_tweet(username):
    ...
    tweet_ids = [tokenizer.encode(str(i), max_length = 5 , pad_to_max_length = True) for i in tweet_csv.cleaned.values]
...
def predict_follow(username):
    ...
    for i in range(5):
        ...
        tweet_ids = [tokenizer.encode(str(k), max_length = 5 , pad_to_max_length = True) for k in new_list]
        tweet_vals = new_model.predict(np.array(tweet_ids))
        tweet_ind = tweet_vals.argmax(axis=1)
